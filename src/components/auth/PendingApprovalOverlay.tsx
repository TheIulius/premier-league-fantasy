...
export const PendingApprovalOverlay: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-md overflow-y-auto animate-fade-in">
      <div className="w-full max-w-md my-auto rounded-3xl bg-slate-900/95 border border-amber-500/30 p-5 sm:p-6 shadow-2xl space-y-4 text-slate-100">
        ...
        {/* Contacts Section */}
        <div className="rounded-2xl bg-slate-950/80 border border-white/10 p-3 space-y-2">
          <p className="text-[11px] font-bold text-slate-300">📞 Organizer Contacts</p>
          <div className="space-y-1.5">
            <p className="text-[11px] text-white font-semibold">Ilia Shinjiashvili</p>
            <button
              type="button"
              onClick={() => copyToClipboard('599100084', 'phone')}
              className="flex items-center gap-1.5 text-xs text-slate-300 hover:text-white transition-colors group cursor-pointer"
            >
              <Phone className="w-3.5 h-3.5 text-emerald-400" />
              <span className="font-mono font-bold">599 100 084</span>
              {copiedField === 'phone' ? (
